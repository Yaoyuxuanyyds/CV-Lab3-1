...
class GetTransformedDataset:
    ...
    def get_cifar10_train(self, n_views):
        return datasets.CIFAR10('/root/Lab3/Lab3-1/data', train=True, 
                                transform=ViewGen(self.get_simclr_transform(32), n_views),
                                download=False)
    
    def get_cifar10_test(self):
        return datasets.CIFAR10('/root/Lab3/Lab3-1/data', train=False,
                                transform=transforms.ToTensor(),
                                download=False)

def get_cifar100_data_loaders(shuffle=False, batch_size=256):
    train_dataset = datasets.CIFAR100('/root/Lab3/Lab3-1/data', train=True, download=False,
                                    transform=transforms.ToTensor())

    train_loader = DataLoader(train_dataset, batch_size=batch_size,
                            num_workers=2, drop_last=False, shuffle=shuffle)
    
    test_dataset = datasets.CIFAR100('/root/Lab3/Lab3-1/data', train=False, download=False,
                                    transform=transforms.ToTensor())
